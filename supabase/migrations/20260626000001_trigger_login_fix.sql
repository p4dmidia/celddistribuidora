-- Migration: Trigger Login Fix
-- Naming: 20260626000001_trigger_login_fix.sql
-- Description: Updates the registration trigger to insert login, full_name, and whatsapp directly into user_profiles upon signup. This avoids RLS blocking the client-side fallback update.
...
CREATE OR REPLACE FUNCTION public.handle_new_affiliate_user()
 ...
 BEGIN
   ...
   -- E. Criar Perfil do Usuário (Popula login, full_name, whatsapp)
   BEGIN
     INSERT INTO public.user_profiles (id, email, role, cpf, cnpj, registration_type, organization_id, sponsor_id, login, full_name, whatsapp, created_at, updated_at)
     VALUES (
       new.id, 
       new.email, 
   ...
   BEGIN
     ...
     VALUES (new.id, new.email, v_full_name, LOWER(new.raw_user_meta_data ->> 'login'), new.raw_user_meta_data ->> 'cpf', new.raw_user_meta_data ->> 'cnpj', new.raw_user_meta_data ->> 'whatsapp', v_org_id, v_sponsor_id, false, new.created_at, new.updated_at);
   EXCEPTION WHEN OTHERS THEN
     GET STACKED DIAGNOSTICS v_error_msg = MESSAGE_TEXT;
